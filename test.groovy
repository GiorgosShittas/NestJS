...
try {
        def response = routeHelper.invokeMicroservice(
                'bil.businessprocess.ms',
                'GET',
                '/processtype' ,
                "processDomain=1",
                Map.class
        );
        log.info("Response of process types API: "+response);
         
        Set<String> processTypesDB= new HashSet<String>(); 
        Set<String> processSubTypesDB= new HashSet<String>(); 

        def processTypeArrayResponse= response.get("processType");
        for (processTypeInstance in processTypeArrayResponse) {
                if(processTypeInstance.containsKey("superType")){
                        processTypesDB.add(processTypeInstance.get("superType").get("processTypeId"));
                        processSubTypesDB.add(processTypeInstance.get("processTypeId"));       
                }
                else{
                        processTypesDB.add(processTypeInstance.get("processTypeId"));
                }
        }
        String[] processTypesDBArray = processTypesDB.toArray(); 
        String[] processSubTypesDBArray = processSubTypesDB.toArray(); 

        log.info("Process Types Array: "+Arrays.toString(processTypesDBArray));
        log.info("Process Subtypes Array: "+Arrays.toString(processSubTypesDBArray));
        log.info("Process Types Array length: "+processTypesDB.size());
        log.info("Process Sub Types Array length: "+processSubTypesDB.size());

    }
    catch (Exception e) {
        log.info("Error while fetching process types");
    }
